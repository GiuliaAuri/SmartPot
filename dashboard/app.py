...
class PlantStatusEvaluator:
    ...
    def determine_plant_status(self, plant_id, sensors):
        """Determine plant status based on sensor data and policies"""
        ...
        if critical_alerts:
            ...
            # Check basic sensor values for healthy status
            battery = SensorDataProcessor.extract_sensor_value(sensors, 'battery_level', 100)
            tank_level = SensorDataProcessor.extract_sensor_value(sensors, 'level_tank', 100)
            
            if battery > 80 and tank_level > 60:
                return "healthy"
            else:
                return "good"
